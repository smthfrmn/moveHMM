...
plot(grid, f(grid),
  type = "l", xlab = "parameter", ylab = "likelihood",
  main = "Local maximum"
)
points(steps1[seq(1, nrow(steps1), by = 1), ],
  type = "o",
  col = "firebrick2", pch = 20, cex = 0.7
)
points(steps1[1, 1], steps1[1, 2],
  pch = "+",
...
legend("topleft",
  legend = c("Starting value", "Estimate"),
...
points(steps2[seq(1, nrow(steps2), by = 1), ],
  type = "o",
  col = "firebrick2", pch = 20, cex = 0.7
)
points(steps2[1, 1], steps2[1, 2],
  pch = "+",
  col = "seagreen", cex = 2.5
)
points(steps2[nrow(steps2), 1], steps2[nrow(steps2), 2],
  pch = "+",
...
# Read data from URL
URL <- paste0(
  "https://www.datarepository.movebank.org/bitstream/handle/",
...
plot(anglegrid, dvm(anglegrid, mu = 0, kappa = kappas[1]),
  type = "l",
...
axis(
  side = 1, at = c(-pi, -pi / 2, 0, pi / 2, pi),
  labels = expression(-pi, -pi / 2, 0, pi / 2, pi)
)
legend("topleft",
  legend = kappas, col = cols, lty = 1,
...
for (i in 1:niter) {
  ...
  anglePar0 <- c(angleMean0, angleCon0)
  allm[[i]] <- fitHMM(
    data = hmmdata, nbStates = 2, stepPar0 = stepPar0,
    anglePar0 = anglePar0
  )
}
@
...
allm_parallel <- parLapply(cl = cl, X = allPar0, fun = function(par0) {
  m <- fitHMM(
    data = hmmdata, nbStates = 2, stepPar0 = par0$step,
