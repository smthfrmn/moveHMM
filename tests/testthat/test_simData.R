...
test_that("Exceptions are thrown", {
  ...
  expect_error(
    simData(1, 2, "gamma", "vm", stepPar, anglePar, nbCovs = 2, zeroInflation = TRUE),
    NA
  )

  expect_error(
    simData(0, 2, "gamma", "vm", stepPar, anglePar, nbCovs = 2, zeroInflation = TRUE),
    "nbAnimals should be at least 1."
  )
  expect_error(
    simData(1, 0, "gamma", "vm", stepPar, anglePar, nbCovs = 2, zeroInflation = TRUE),
  ...
  expect_error(
    simData(1, 2, "gamma", "vm", stepPar2, anglePar, nbCovs = 2, zeroInflation = TRUE),
    "Wrong number of parameters"
  )
  anglePar2 <- c(anglePar, 1)
  expect_error(
    simData(1, 2, "gamma", "vm", stepPar, anglePar2, nbCovs = 2, zeroInflation = TRUE),
    "Wrong number of parameters"
  )
