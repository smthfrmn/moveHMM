...
test_that("Exceptions are thrown", {
  ...
  bounds <- parDef(
    simPar$stepDist, simPar$angleDist, simPar$nbStates,
    estAngleMean, simPar$zeroInflation
  )$bounds
  parSize <- parDef(
    simPar$stepDist, simPar$angleDist, simPar$nbStates,
    estAngleMean, simPar$zeroInflation
  ...
  expect_error(nLogLike(
    wpar, simPar$nbStates, bounds, parSize, data, simPar$stepDist, simPar$angleDist,
    simPar$angleMean, simPar$zeroInflation
  ), NA)

  # if not enough parameters provided
  expect_error(nLogLike(
    wpar[-1], simPar$nbStates, bounds, parSize, data, simPar$stepDist, simPar$angleDist,
    simPar$angleMean, simPar$zeroInflation
  ))

  # if stepDist not in list
  expect_error(nLogLike(
    wpar, simPar$nbStates, bounds, parSize, data, "unif", simPar$angleDist,
    simPar$angleMean, simPar$zeroInflation
  ))

  # if angleDist not in list
  expect_error(nLogLike(
    wpar, simPar$nbStates, bounds, parSize, data, simPar$stepDist, "norm",
    simPar$angleMean, simPar$zeroInflation
  ))
...
test_that("angleMean = NULL, angleDist = NULL, and zeroInflation = TRUE work", {
  ...
  expect_error(nLogLike(
    wpar, simPar$nbStates, bounds, parSize, data, simPar$stepDist, "none",
    NULL, TRUE
  ), NA)
})
...
test_that("logAlpha, logBeta, and nLogLike are consistent", {
  ...
  nll <- nLogLike(
    m$mod$estimate, simPar$nbStates, bounds, parSize, data, simPar$stepDist, simPar$angleDist,
    simPar$angleMean, simPar$zeroInflation
  )
