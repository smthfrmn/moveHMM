...
fitHMM <- function(data, nbStates, stepPar0, anglePar0 = NULL, beta0 = NULL, delta0 = NULL, formula = ~1,
  ...
  is.formula <- function(x) {
    tryCatch(inherits(x, "formula"), error = function(e) {
      FALSE
    })
  }
  ...
  if (!is.null(beta0)) {
    if (ncol(beta0) != nbStates * (nbStates - 1) | nrow(beta0) != nbCovs + 1) {
      error <- paste(
        "beta0 has wrong dimensions: it should have", nbCovs + 1, "rows and",
  ...
  if (length(which(stepPar0 <= stepBounds[, 1] | stepPar0 >= stepBounds[, 2])) > 0) {
    stop(paste(
      "Check the step parameters bounds (the initial parameters should be",
  ...
  if (fit) {
    ...
    ) # filter warnings using function h
  ...
  if (stationary) {
    ...
    tryCatch(
      ...
      error = function(e) {
        stop(paste(
          "A problem occurred in the calculation of the stationary",
  ...
  conditions <- list(
    stepDist = stepDist, angleDist = angleDist, zeroInflation = zeroInflation,
    estAngleMean = estAngleMean, stationary = stationary, formula = formula
  )

  mh <- list(
    data = data, mle = mle, mod = mod, conditions = conditions,
