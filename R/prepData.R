...
prepData <- function(trackData, type = c("LL", "UTM"), coordNames = c("x", "y"), LLangle = NULL) {
  ...
  if ("ID" %in% colnames(trackData)) {
    ID <- as.character(trackData$ID)
  } # homogenization of numeric and string IDs
  else {
    ID <- rep("Animal1", nrow(trackData))
  ...
  if (is.null(LLangle)) {
    LLangle <- type == "LL"
  } # TRUE if type=='LL', FALSE otherwise
  ...
  for (zoo in unique(ID)) {
    if (length(which(ID == zoo)) < 2) {
      ...
      ID <- ID[-which(ID == zoo)]
      warning(paste(
        "Track", zoo, "only contains one observation,",
  ...
  for (zoo in 1:nbAnimals) {
    ...
    for (i in (i1 + 1):(i2 - 1)) {
      ...
      if (!is.na(x[i - 1]) & !is.na(x[i]) & !is.na(y[i - 1]) & !is.na(y[i])) {
        # step length
        step[i - i1] <- spDistsN1(
          pts = matrix(c(x[i - 1], y[i - 1]), ncol = 2),
          pt = c(x[i], y[i]),
          longlat = (type == "LL")
        ) # TRUE if 'LL', FALSE otherwise
      }
    ...
    if (!is.na(x[i2 - 1]) & !is.na(x[i2]) & !is.na(y[i2 - 1]) & !is.na(y[i2])) {
      step[i2 - i1] <- spDistsN1(
        pts = matrix(c(x[i2 - 1], y[i2 - 1]), ncol = 2),
        pt = c(x[i2], y[i2]),
        longlat = (type == "LL")
      ) # TRUE if 'LL', FALSE otherwise
    }
  ...
  if (length(covsCol) > 0) {
    ...
    if (length(which(is.na(covs))) > 0) {
      warning(paste(
        "There are", length(which(is.na(covs))),
