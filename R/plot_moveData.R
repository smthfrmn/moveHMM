...
plot.moveData <- function(x, animals = NULL, compact = FALSE, ask = TRUE, breaks = "Sturges", ...) {
  ...
  if (is.null(data$angle) | all(data$y == 0)) {
    ...
    for (zoo in animalsInd) {
      ...
      plot(step,
        type = "l", xlab = "t", ylab = "step length",
    ...
    for (zoo in animalsInd) {
      ...
      par(mfrow = c(2, 2))
      plot(step,
        type = "h", xlab = "time", ylab = "step length",
        ylim = c(0, max(step, na.rm = T))
      )
      plot(angle,
        type = "h", xlab = "time", ylab = "turning angle (radians)",
        ylim = c(-pi, pi), yaxt = "n"
      )
      axis(2,
      ...
      #################################
      hist(step,
        xlab = "step length", main = "", breaks = breaks,
        col = "grey", border = "white"
      )

      h <- hist(angle, breaks = breaks, plot = FALSE) # to define the breaks
      hist(angle,
        xlab = "turning angle (radians)", main = "",
